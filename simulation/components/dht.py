...
from simulator.dht import run_dht_simulator
import threading
import time
...
def run_dht(settings, threads, stop_event):
        if settings['simulated']:
            ...
            from sensors.dht import run_dus_loop, DHT
            print("Starting dht1 loop")
            dht = DHT(settings['pin'])
            dht1_thread = threading.Thread(target=run_dus_loop, args=(dht, 2, dht_callback, stop_event))
            dht1_thread.start()
            threads.append(dht1_thread)
