...
def run_ds2_loop(ds2, delay, callback, stop_event, publish_event, settings):
    while True:
        ds2.press()
        if ds2.start_time is not None:
            elapsed_time = time.time() - ds2.start_time
            if elapsed_time >= 5:
                ds2.mqtt_client.publish("alarm", "on")

        callback(publish_event, settings)
        if stop_event.is_set():
            ds2.release()  # Ensure that the release method is called before breaking the loop
            break
        time.sleep(delay)
